...
TEST start_hashtable(void) {
  struct tablec_oa_ht tablec;
  tablec_oa_init(&tablec, 16);
...
TEST setting_key(void) {
  ...
  ASSERT_EQm("TableC was not able to set a key and a value.", strcmp((char *)tablec_oa_get(&tablec, key)->value, value), 0);
...
TEST setting_key_and_read_after_deleting(void) {
  ...
  tablec_oa_del(&tablec, key);
  ASSERT_EQm("TableC was not able to delete key.", tablec_oa_get(&tablec, key) == NULL, 1);
...
TEST hashtable_overflow(void) {
  struct tablec_oa_ht tablec;
  tablec_oa_init(&tablec, 1);

  ASSERT_EQm("TableC was not able to detect that the hashtable is full.", tablec_oa_full(&tablec), 1);
  ...
  tablec_oa_set(&tablec, otherKey, value);
  ASSERT_EQm("TableC was not able to set a key and a value.", strcmp((char *)tablec_oa_get(&tablec, otherKey)->value, value), 0);
...
TEST test_resize(void) {
  ...
  while(i++ < 1000) {
    ...
    tablec_oa_resize(&tablec, tablec.capacity + 1);
    tablec_oa_set(&tablec, keyRandom, value);
    ASSERT_EQm("TableC was not able to set a key and a value.", strcmp((char *)tablec_oa_get(&tablec, keyRandom)->value, value), 0);
  }
